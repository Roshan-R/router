...
use crate::query_planner::convert::convert_query_plan_from_fed_next;
    ...
    async fn plan(
        ...
        match self {
            ...
            PlannerMode::Rust { rust, .. } => {
                ...
                Ok(convert_query_plan_from_fed_next(plan))
            }
            PlannerMode::Both { js, rust } => {
                ...
                let rust_result = rust
                    .build_query_plan(&document, operation.as_deref())
                    .map(convert_query_plan_from_fed_next);
            ...
            match add_defer_labels(schema, &doc.ast) {
                ...
                Ok(modified_query) => {
                    let executable_document = modified_query
                        ...
                        .map_err(|e| SpecError::ValidationError(e.errors.into()))?;
